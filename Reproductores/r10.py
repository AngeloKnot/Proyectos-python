...
class ReproductorMusica:
    def __init__(self, ventana_principal):
        ...
        self.ventana_principal.title("Reproductor de Música Completo")
        ...
        self.boton_abrir_carpeta.grid(row=0, column=0, padx=10, pady=20)
        ...
        self.boton_reproducir.grid(row=0, column=1, padx=10, pady=20)

        self.boton_pausar = tk.Button(self.ventana_principal, text="Pausar", command=self.pausar_musica, 
                                      state=tk.DISABLED, bg="#FF5722", fg="white", font=("Arial", 12), padx=10, pady=5)
        self.boton_pausar.grid(row=0, column=2, padx=10, pady=20)
        ...
        self.etiqueta_cancion.grid(row=1, column=0, columnspan=3, pady=10)

        # Metadatos
        self.etiqueta_metadatos = tk.Label(self.ventana_principal, text="Artista: -, Título: -, Año: -, Género: -", 
                                           bg="#1E1E1E", fg="white", font=("Arial", 12))
        self.etiqueta_metadatos.grid(row=2, column=0, columnspan=3, pady=5)
        ...
        self.barra_volumen.grid(row=3, column=0, columnspan=3, pady=10)
        ...
        self.barra_progreso.grid(row=4, column=0, columnspan=3, pady=10)

        # Botones de adelantar y retroceder
        self.boton_retroceder = tk.Button(self.ventana_principal, text="Retroceder", command=self.retroceder, 
                                          bg="#FFC107", fg="black", font=("Arial", 12), padx=10, pady=5)
        self.boton_retroceder.grid(row=5, column=0, padx=20)

        self.boton_adelantar = tk.Button(self.ventana_principal, text="Adelantar", command=self.adelantar, 
                                         bg="#FFC107", fg="black", font=("Arial", 12), padx=10, pady=5)
        self.boton_adelantar.grid(row=5, column=1, padx=20)
        ...
        self.boton_aleatorio = tk.Button(self.ventana_principal, text="Modo Aleatorio", command=self.toggle_aleatorio, 
                                         bg="#FFC107", fg="black", font=("Arial", 12), padx=10, pady=5)
        self.boton_aleatorio.grid(row=6, column=0, padx=20, pady=10)
        ...
        self.boton_repetir = tk.Button(self.ventana_principal, text="Repetir Canción", command=self.toggle_repetir, 
                                       bg="#FFC107", fg="black", font=("Arial", 12), padx=10, pady=5)
        self.boton_repetir.grid(row=6, column=1, padx=20, pady=10)

        # Mostrar onda de sonido
        self.boton_onda = tk.Button(self.ventana_principal, text="Mostrar Onda", command=self.mostrar_onda, 
                                     bg="#4CAF50", fg="white", font=("Arial", 12), padx=10, pady=5)
        self.boton_onda.grid(row=7, column=0, columnspan=3, pady=10)

        # Barra de búsqueda
        self.entrada_busqueda = tk.Entry(self.ventana_principal, font=("Arial", 12))
        self.entrada_busqueda.grid(row=8, column=0, padx=10, pady=10)
        ...
        self.boton_buscar.grid(row=8, column=1, padx=10, pady=10)

        # Lista de canciones
        self.lista_reproduccion = tk.Listbox(self.ventana_principal, height=10, width=50, font=("Arial", 12))
        self.lista_reproduccion.grid(row=9, column=0, columnspan=3, pady=10)
        ...
        self.boton_seleccionar = tk.Button(self.ventana_principal, text="Seleccionar Canción", command=self.seleccionar_cancion, 
                                           bg="#2196F3", fg="white", font=("Arial", 12), padx=10, pady=5)
        self.boton_seleccionar.grid(row=10, column=0, columnspan=3, pady=10)
    ...
    def mostrar_metadatos(self):
        ...
        artista = audio.get("TPE1", None)
        titulo = audio.get("TIT2", None)
        ano = audio.get("TDRC", None)
        genero = audio.get("TCON", None)

        artista = artista.text[0] if artista else "-"
        titulo = titulo.text[0] if titulo else "-"
        ano = ano.text[0] if ano else "-"
        genero = genero.text[0] if genero else "-"

        self.etiqueta_metadatos.config(text=f"Artista: {artista}, Título: {titulo}, Año: {ano}, Género: {genero}")
    ...
    def mostrar_onda(self):
        ...
        self.canvas.get_tk_widget().grid(row=11, column=0, columnspan=3, pady=10)
        self.canvas.draw()
    ...
    def retroceder(self):
        self.cancion_indice = (self.cancion_indice - 1) % len(self.archivos_canciones)
